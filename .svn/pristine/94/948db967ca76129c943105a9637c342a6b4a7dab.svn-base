...
public class EinAusgabe implements Viewer, Requestable {
	 ...
	 *	Spielstein-Legende:	<br>0(white)			--- leeres Feld<br>
	 *							0(farbig)			--- Bauer<br>
	 *							rote Symbole		--- rote Spielsteine<br>
	 *							blaue Symbole		--- blaue Spielsteine<br>
	 *							Nummer (!= 0)		--- Turm<br>
	 *							Nummber*			--- geblockter Turm<br>
	 *							x					--- Basis
	...
	public String toString() {
		...
		//Gibt den letzten Zug aus, falls dieser existiert und den Status des Boards
		if(!board.getSavedMoves().isEmpty())
			System.out.println("Letzter Zug: "+ (Move)board.getSavedMoves().getLast() +"\nBoardStatus: " + board.getStatus());
	...
	*/
	...
	public Move deliver() {
		...
		while(ready == false) {
			...
			if(!board.isLegal(nextMove)) {
				ready = false;
				System.err.println("Illegalen Move eingegeben");
			}
			
			//gibt den letzten Zug aus, der im SavedMove gespeichert wird
		}
		return nextMove;
	}
	 ...
	 *	@return String mit fertig gedrawter Spaltenbeschriftung
	 */
