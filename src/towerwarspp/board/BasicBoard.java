...
public class BasicBoard implements Viewable{
    ...
    private void updateState(Status status) {
		...
		if(!hasLegalMove()) {
			this.status = (currentPlayer == PlayerColor.RED) ? Status.BLUE_WIN : Status.RED_WIN;
		}
    }
    ...
    /** 
     *  Hilfsmethode, die die Nachbarschaft von einer Position ermittelt. Siehe @see http://www.redblobgames.com/grids/hexagons/
     *  @param pos Position
	...
	protected LinkedList<Move> legalMoves(boolean all) {
		...
		Move move;
		Token startToken;
		//Iteriert ueber alle möglichen Token
